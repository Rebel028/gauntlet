...
4. **Spawn them in parallel.** Independence is the whole point — the agents must not see each other's reasoning. Delegate to all the subagents in one turn so they run concurrently and can't influence one another.
